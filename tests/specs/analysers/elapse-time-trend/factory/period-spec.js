describe('Elapse time trend period factory', function() {
    ...
    it('calculates the gaps between each number draw in a single `period`', function() {
        ...
        assert(period.hits[14], 15, [1, 13], 3, 7);
        ...
        assert(period.hits[23], 24, [], , );
        assert(period.hits[24], 25, [], , );
        assert(period.hits[25], 26, [], , );
        assert(period.hits[26], 27, [], , );
        assert(period.hits[27], 28, [], , );
        assert(period.hits[28], 29, [], , );
        assert(period.hits[29], 30, [], , );
        assert(period.hits[30], 31, [], , );
        assert(period.hits[31], 32, [], , );
        assert(period.hits[32], 33, [], , );
        assert(period.hits[33], 34, [], , );
        assert(period.hits[34], 35, [], , );
        console.log(period);
    });
});